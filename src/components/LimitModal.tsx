...
export default function LimitModal({ isOpen, onClose, onPay }: { isOpen: boolean, onClose: () => void, onPay: () => void }) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[99999] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
