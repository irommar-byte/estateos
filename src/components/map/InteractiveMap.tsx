...
import { Search, Building2, SlidersHorizontal, MapPin, Maximize, Lock, UserCheck, ShieldCheck, Crown, User } from "lucide-react";
...
export default function InteractiveMap() {
  ...
  useEffect(() => {
    fetch('/api/user/profile').then(res => res.json()).then(user => { if (user && user.email) setIsLoggedIn(true); }).catch(() => setIsLoggedIn(false));
  }, []);
  ...
  return (
    <div className="w-full bg-[#050505] py-24 relative">
      ...
      <AnimatePresence>
        {showTeaser && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[1000] bg-black/60 backdrop-blur-md flex items-center justify-center p-6">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-[#0a0a0a] border border-white/10 p-10 rounded-[3rem] max-w-lg w-full shadow-[0_0_100px_rgba(0,0,0,1)] relative text-center">
              ...
              <div className="grid grid-cols-1 gap-4 mb-10 text-left">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  ...
                  <span className="text-sm font-bold text-white/80 uppercase tracking-widest">Dostęp do ukrytych ofert</span>
                </div>
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <UserCheck className="text-emerald-500" size={24} />
                  <span className="text-sm font-bold text-white/80 uppercase tracking-widest">Bezpośredni czat z właścicielem</span>
                </div>
              </div>
