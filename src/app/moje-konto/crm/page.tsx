...
import { ShieldCheck, ChevronLeft, ArchiveX, Calendar, Crown, Plus, Phone, CheckCircle, Loader2, Star, ChevronDown, Building2, DollarSign, Wallet, X, Radar, Send, Clock, FileText, Lock, Unlock, Activity, TrendingUp, Wifi, RefreshCcw, Sparkles, Edit2, ExternalLink, Home, Key, LayoutGrid, CalendarDays, SlidersHorizontal, MapPin, Target, Heart } from 'lucide-react';
...
export default function CRMDashboard() {
  ...
  // Niezależne, bezpieczne pobieranie opinii w tle
  useEffect(() => {
  ...
  const [radarFormData, setRadarFormData] = useState({ searchDistricts: [] as string[], searchRooms: '', searchAreaFrom: '', searchMaxPrice: '' });
  ...
  const handleSaveRadar = async (e: React.FormEvent) => {
    ...
    try {
        const res = await fetch('/api/szukaj/aktualizuj', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
               districts: radarFormData.searchDistricts,
               rooms: radarFormData.searchRooms,
               areaFrom: radarFormData.searchAreaFrom,
               maxPrice: radarFormData.searchMaxPrice
            })
        });
        if (res.ok) {
            setIsEditRadarOpen(false);
            setIsRadarUpdating(true); // Odpalamy luksusową animację
  ...
  // 🔥 RELACYJNA BAZA DANYCH PREMIUM (Mock Data symulujący produkcję) 🔥
  
  // 1. BAZA UŻYTKOWNIKÓW (18 zweryfikowanych kont)
  const mockUsers = [
    // --- 10 WŁAŚCICIELI (Sellers) ---
    ...
    // --- 8 INWESTORÓW (Buyers) z ustawieniami Radaru ---
  ...
  // 2. BAZA OFERT (10 nieruchomości powiązanych twardo z Właścicielami)
  const relationalOffers = [
    ...
    { id: 'offer-005', sellerId: 'usr-s05', title: 'Willa z prywatnym basenem', price: 12500000, location: 'Kraków, Wola Justowska', area: 450, rooms: 8, imageUrl: 'https://images.unsplash.com/photo-1613490908592-5d3164c4c11b?w=800&q=80', createdAt: '2026-03-21T17:08:14.732Z', expiresAt: '2026-03-22T00:41:34.732Z' }, // WYGASŁA
  ...
  // 3. INICJALIZACJA STANU CRM DLA ZALOGOWANEGO UŻYTKOWNIKA
  const [crmData, setCrmData] = useState<any>({ 
    offers: relationalOffers, 
    contacts: mockUsers, 
    appointments: [], bids: [], leadTransfers: [] 
  });
  
  // 🔥 PAMIĘĆ ULUBIONYCH (Symulujemy, że Inwestor polubił już 2 wybrane oferty: Złotą 44 i Rezydencję na Mazurach)
  const [likedOfferIds, setLikedOfferIds] = useState<string[]>(['offer-001', 'offer-007']);



  // --- RADAR POWIADOMIEŃ (Deep Link z Dzwoneczka) ---
  useEffect(() => {
     if (typeof window !== 'undefined' && crmData?.appointments) {
        ...
        if (appIdFromUrl) {
           ...
           if (foundApp) {
              setManagingApp(foundApp); // Otwiera zielone okienko negocjacji
              
              // Czyścimy link, żeby odświeżenie strony nie otwierało okienka w nieskończoność
              const newUrl = window.location.protocol + '//' + window.location.host + window.location.pathname;
              window.history.replaceState({path: newUrl}, '', newUrl);
           }
        }
     }
  }, [crmData]); // Radar uruchamia się, gdy CRM zaciągnie świeże spotkania
  // --------------------------------------------------
  ...
  const handleArchiveSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchData(); // Błyskawiczne odświeżenie danych w tle
      } else {
  ...
  const handleSendVip = async (offerId: number, buyerIds: number[]) => {
    try { 
      const res = await fetch('/api/crm/radar/send', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ offerId, buyerIds }) }); 
  ...
  const fetchData = async () => {
    try {
      ...
      if (!data.error) setCrmData({ ...data, bids: data.bids || [], leadTransfers: data.leadTransfers || [] });
  ...
  useEffect(() => { 
    fetch('/api/auth/check').then(r => r.json()).then(d => {
       // TWARDA BLOKADA: Wyrzucenie niezalogowanych "duchów"
       if (!d.loggedIn) {
           window.location.href = '/login';
           return;
       }
       if(d.loggedIn) {
           fetch('/api/user/profile').then(r=>r.json()).then(uData => {
               setCurrentUser(uData);
               
               // LOGIKA ZAPŁONU: Tylko dla PRO i tylko raz na sesję
               if (uData.isPro && !sessionStorage.getItem('pro_booted')) {
                   ...
                   // Kończymy bootowanie po 3 sekundach (wystarczy na animację)
    ...
    if (searchParams && searchParams.get('payment_success') === 'true') {
       ...
       if (plan === 'renewal') {
           ...
           fetch('/api/stripe/force-sync', {
               ...
               fetchData();
               fetch('/api/user/profile').then(r=>r.json()).then(uData => setCurrentUser(uData));
           });
       }
    ...
    fetchData(); 
    fetchRadarData(); // Zawsze pobieramy Radar, niezależnie od bycia PRO!
    ...
    return (
        <div className="fixed inset-0 z-[999999] bg-[#050505] flex flex-col items-center justify-center font-sans overflow-hidden">
           ...
           <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1, ease: "easeOut" }} className="relative z-10 flex flex-col items-center">
              ...
              {/* Odpalający się zegar jako centralny punkt testu */}
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[#111] via-[#050505] to-black text-white p-6 pt-32 pb-40 font-sans relative">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* 🔥 LUKSUSOWE POWITANIE UŻYTKOWNIKA 🔥 */}
        ...
        {/* 🔥 PRZEŁĄCZNIK TRYBÓW (Apple-style) 🔥 */}
        ...
        {/* 🔥 DYNAMICZNY, POZIOMY BANNER PREMIUM (Dla wszystkich zakładek) 🔥 */}
        <motion.div
          key={activeTab + mode} // Wymusza odtworzenie animacji wejścia przy każdej zmianie
        ...
        >
          ...
          {/* Kontener Animowanej Ikony z Lewej Strony */}
  ...
  <div className="relative w-full h-full flex items-center justify-center overflow-hidden rounded-full perspective-1000">
    ...
    {/* Animacja omiatającego Radaru (Conic Gradient) */}
    <motion.div 
      animate={{ rotate: 360 }} 
  ...
  <div className="relative w-full h-full flex items-center justify-center perspective-[800px]">
    ...
    {/* Lewitująca, obracająca się w 3D ikona */}
    <motion.div
      animate={{ y: [-3, 3, -3], rotateX: [0, 15, 0], rotateY: [-10, 10, -10] }}
    ...
    {/* Mechaniczne koła i świetlne pulsowanie */}
  ...
  <div className="relative w-full h-full flex items-center justify-center perspective-1000">
    ...
    {/* Baza Kalendarza w 3D */}
    <motion.div 
      animate={{ rotateY: [-5, 5, -5] }} 
      transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
      className="relative z-10 w-12 h-12 bg-[#1a1a1a] border border-purple-500/40 rounded flex flex-col items-center pt-1 drop-shadow-[0_8px_10px_rgba(0,0,0,0.8)]"
    >
      {/* Kółka segregatora */}
      ...
      {/* Animowana, odrywająca się kartka */}
      <motion.div 
        animate={{ rotateX: [0, 0, -110, -110], opacity: [1, 1, 0, 0] }} 
        transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
        className="absolute bottom-0 w-full h-[70%] bg-gradient-to-b from-[#2a2a2a] to-[#1a1a1a] border-t border-purple-500/30 origin-top flex items-center justify-center overflow-hidden rounded-b z-20"
      >
        {/* Animowane rysowanie zaznaczenia (Ptaszek) */}
      ...
      {/* Spód kalendarza (następny dzień odkrywany po wyrwaniu kartki) */}
    ...
    {/* Latający, animowany ołówek ze złota */}
    <motion.div 
      animate={{ 
        x: [12, -2, 8, 16, 12], 
        y: [-12, -2, 4, -8, -12],
        rotateZ: [-10, -30, -10, 10, -10]
      }} 
      transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
      className="absolute z-30 text-amber-400 drop-shadow-[0_5px_8px_rgba(0,0,0,0.7)]"
    >
    ...
    {/* Zewnętrzny pierścień przestrzenny */}
    <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: 'linear' }} className="absolute -inset-2 border-2 border-transparent border-t-purple-500/40 border-b-purple-500/10 rounded-full shadow-[0_0_10px_rgba(168,85,247,0.2)]" />
          ...
          {/* Teksty Zmieniające się w locie */}
        ...
        {/* 🔥 WIDGET: USTAWIENIA I WYNIKI RADARU 🔥 */}
        {activeTab === 'radar' && (
          <div className="flex flex-col gap-8 mb-12">
            
            
            {/* 🔥 NOWY LUKSUSOWY WIDGET RADARU + MODALE 🔥 */}
            <>
            <div className="relative w-full mb-12 p-8 md:p-10 rounded-[3rem] border border-white/5 bg-gradient-to-br from-[#111111] to-[#050505] shadow-[inset_0_0_80px_rgba(0,0,0,0.8),0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden group transition-all duration-700 hover:shadow-[inset_0_0_80px_rgba(0,0,0,0.9),0_30px_60px_rgba(16,185,129,0.1)]">
              ...
              <div className="relative z-10 flex flex-col md:flex-row gap-8 justify-between items-start md:items-center border-b border-white/5 pb-8">
                ...
                <button onClick={openRadarEditor} className="relative px-8 py-4 rounded-full bg-[#1a1a1a] border border-white/10 hover:border-emerald-500/50 text-white text-xs font-black uppercase tracking-widest overflow-hidden transition-all duration-500 hover:shadow-[0_0_30px_rgba(16,185,129,0.2)] group/btn cursor-pointer">
                  <div className="absolute inset-0 bg-emerald-500/10 translate-y-full group-hover/btn:translate-y-0 transition-transform duration-500 ease-out" />
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[99999] bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
                    <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} className="bg-[#0a0a0a] border border-white/10 rounded-[2.5rem] w-full max-w-lg p-8 shadow-2xl relative overflow-hidden">
                        ...
                        <form onSubmit={handleSaveRadar} className="space-y-6 z-10 relative">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="number" min="1" className="w-full bg-[#111] border border-white/10 rounded-xl px-5 pl-12 py-4 text-white font-black text-lg outline-none focus:border-emerald-500 transition-colors" placeholder="np. 40" value={radarFormData.searchAreaFrom} onChange={e => setRadarFormData({...radarFormData, searchAreaFrom: e.target.value})} />
                                    </div>
                                </div>
                                <div>
                                    <label className="text-[10px] uppercase tracking-[0.2em] text-white/50 font-bold block mb-2">Liczba Pokoi (Select)</label>
                                    <div className="relative">
                                        <SlidersHorizontal size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30 pointer-events-none" />
                            ...
                            <button type="submit" disabled={isSavingRadar} className="w-full mt-4 py-5 bg-emerald-500 text-black font-black uppercase tracking-widest rounded-xl hover:bg-emerald-400 transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] disabled:opacity-50 cursor-pointer">
                                {isSavingRadar ? 'ZAPISYWANIE...' : 'ZAKTUALIZUJ RADAR'}
                            </button>
                        </form>
                    </motion.div>
                </motion.div>
            ...
            {/* PEŁNOEKRANOWA ANIMACJA REKALIBRACJI */}
            <AnimatePresence>
              {isRadarUpdating && (
        ...
        {activeTab === 'offers' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(!crmData.offers || (mode === 'BUYER' ? crmData.offers.filter((o:any) => likedOfferIds.includes(o.id.toString())) : crmData.offers).length === 0) ? (
              <div className="col-span-full flex flex-col items-center justify-center py-24 border border-dashed border-white/10 rounded-[2.5rem] bg-[#0a0a0a] relative overflow-hidden shadow-[inset_0_0_50px_rgba(0,0,0,0.8)]">
                ...
                {mode === 'SELLER' && (
                  <motion.button
                    ...
                    className="relative z-10 flex items-center gap-3 px-8 py-4 bg-blue-600/20 border border-blue-500/50 hover:bg-blue-600 hover:border-blue-500 text-white rounded-full font-black uppercase tracking-wider text-sm transition-all duration-300"
                  >
              ...
              /* Genialny trik: wstrzykujemy sztuczny obiekt 'isDummy' na sam początek listy, jeśli to Sprzedający! */
              [...(mode === 'SELLER' ? [{ id: 'ADD_NEW_BTN', isDummy: true }] : []), ...(mode === 'BUYER' ? crmData.offers.filter((o:any) => likedOfferIds.includes(o.id.toString())) : crmData.offers)].map((offer: any) => {
                ...
                const isNew = (now.getTime() - createdAt.getTime()) < (1000 * 60 * 60 * 24); // Nowe przez 24h
                ...
                return (
                  <div key={offer.id} className={`bg-[#0a0a0a] border rounded-[2.5rem] p-6 relative overflow-hidden transition-all duration-300 shadow-xl group ${isArchived ? 'border-red-500/20 opacity-90' : 'border-white/10 hover:border-emerald-500/30 hover:shadow-[0_20px_40px_-15px_rgba(16,185,129,0.2)] hover:-translate-y-1'}`}>
                    ...
                    {/* Lewitujące Serduszko (Tylko dla Inwestora) */}
                    ...
                    <div className="flex gap-4 mb-6 relative z-10">
                      ...
                      {/* Kontener Tekstu (z min-w-0 żeby zapobiec wypychaniu) */}
                      <div className="flex-1 min-w-0 flex flex-col justify-center">
                        <div className="flex justify-between items-start gap-2 mb-1">
                          ...
                          {/* Plakietki (ze shrink-0 żeby ich nie ścisnęło) */}
                          <div className="shrink-0">
                            ...
                            ) : (offer.status === 'pending' || offer.status === 'pending_approval' || offer.status === 'in_review') ? (
        ...
        {activeTab === 'planowanie' && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="bg-[#0a0a0a] border border-white/10 rounded-[2.5rem] p-6 md:p-8 shadow-2xl relative overflow-hidden">
              ...
              {/* Siatka Dni (Podobieństwo do Apple Calendar) */}
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3">
                {Array.from({ length: 14 }).map((_, i) => {
                  const d = new Date();
                  d.setDate(d.getDate() + i);
                  
                  // Symulacja danych na podstawie istniejących umówień
    ...
    <div className="flex flex-col">
        {(() => {
          ...
          {/* --- GWARANTOWANY MODAL NEGOCJACJI --- */}
          <AnimatePresence>
            {managingApp && (() => {
               // Generowanie dat i godzin (dokładnie tak jak w AppointmentModal)
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 999999, backgroundColor: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
                 <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} style={{ backgroundColor: '#0a0a0a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '2.5rem', width: '100%', maxWidth: '500px', boxShadow: '0 50px 100px rgba(0,0,0,1)', overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
                    ...
                    <div style={{ padding: '32px', position: 'relative', zIndex: 10, flexShrink: 0 }}>
                       ...
                       {/* NAGŁÓWEK Z OPCJĄ COFANIA KROKU */}
                       ...
                       {/* DANE SPOTKANIA (Ukrywamy w trakcie zmiany terminu by zyskać miejsce) */}
                       <AnimatePresence>
                          {!isRescheduling && (
                    ...
                    {/* INTERAKTYWNY KALENDARZ I PRZYCISKI (Obszar przewijany) */}
    ...
    try {
        ...
        if(res.ok) {
            ...
            setTimeout(() => window.location.reload(), 1500); // 1.5s na animację sukcesu
                             ...
                             <motion.div key="calendar" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
                                ...
                                {/* KROK 3: POTWIERDZENIE (Przycisk Apple Style) */}
                                {rescheduleStep === 3 && (
    ...
    try {
        ...
        if(res.ok) {
          ...
          {/* --- GWARANTOWANY MODAL PROFILU PUBLICZNEGO --- */}
          <AnimatePresence>
            {viewingProfile && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999999, backgroundColor: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(20px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }} onClick={() => setViewingProfile(null)}>
                 <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} style={{ backgroundColor: '#0a0a0a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '2.5rem', width: '100%', maxWidth: '400px', boxShadow: '0 50px 100px rgba(0,0,0,1)', overflow: 'hidden', position: 'relative', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }} onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="custom-scrollbar" style={{ padding: '40px 32px 32px 32px', overflowY: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', position: 'relative', zIndex: 10 }}>
                        ...
                        {/* DYNAMICZNE STATYSTYKI STAWIENNICTWA */}
                        ...
                        {/* DYNAMICZNA LISTA OFERT UŻYTKOWNIKA */}
                 ...
                 {/* UKRYTY MODAL Z OPINIAMI (Wywoływany po kliknięciu w gwiazdki) */}
      ...
      {/* MODAL ZAKOŃCZENIA SPRZEDAŻY (APPLE STYLE) */}
      <AnimatePresence>
        {offerToArchive && (
      ...
      {/* EFEKT WOW: Luksusowy Modal Opinii */}
      <ReviewsModal 
