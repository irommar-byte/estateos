...
import { Lock, Loader2, AlertCircle, Mail, Key, ArrowLeft, CheckCircle } from "lucide-react";
      ...
      return (
        <motion.form key="forgot" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} onSubmit={handleRequestReset} className="bg-[#0a0a0a] border border-white/10 rounded-[2rem] p-8 space-y-6 shadow-2xl">
          <div className="mb-2">
             ...
             <p className="text-xs text-white/50 leading-relaxed">Wpisz adres E-mail powiązany z Twoim kontem. Wyślemy Ci 6-cyfrowy kod autoryzacyjny.</p>
          </div>
          <div>
            <label className="text-[10px] font-bold text-white/40 uppercase tracking-[0.2em] block mb-2 flex items-center gap-2"><Mail size={14}/> E-mail konta</label>
            <input type="email" required placeholder="jan@kowalski.pl" className="w-full bg-transparent text-2xl border-b border-white/10 pb-2 focus:border-emerald-500 transition-colors outline-none" onChange={(e) => setResetEmail(e.target.value)} value={resetEmail} />
          </div>
      ...
      return (
        <motion.form key="reset" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} onSubmit={handleConfirmReset} className="bg-[#0a0a0a] border border-emerald-500/30 rounded-[2rem] p-8 space-y-6 shadow-[0_0_40px_rgba(16,185,129,0.1)]">
          <div className="mb-2">
             ...
             <p className="text-xs text-white/50 leading-relaxed">Kod został wysłany na adres <b>{resetEmail}</b>. Wpisz go poniżej wraz z nowym hasłem.</p>
          </div>
          <div>
            <label className="text-[10px] font-bold text-white/40 uppercase tracking-[0.2em] block mb-2 flex items-center gap-2"><Key size={14}/> 6-cyfrowy kod z e-maila</label>
